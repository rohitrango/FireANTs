...
def main(args):
    ...
    if args.init_template_path is not None:
        ...
        # init image files
        image_files = get_image_files(args)
        ...
        with Timer('Averaging all images for initial template', logger_zero):
            ...
            del init_template.array
            init_template.array = init_template_arr
            if debug and local_rank == 0:
                torch.save(init_template_arr, "tmp/init_template.pt")
    ...
    for epoch in range(args.template_iterations):
        ...
        # new template array and batchified
        updated_template_arr = torch.zeros_like(init_template.array, device=device)
        init_template_batch = BatchedImages([init_template])

        # run through batches
        for batch in imgbatches:
            ...
            updated_template_arr += moved_images.detach().sum(0, keepdim=True)
            del moved_images
        
        # update template
        dist.all_reduce(updated_template_arr / total_file_count, op=dist.ReduceOp.SUM)
        # save here
        if debug and local_rank == 0:
            torch.save(updated_template_arr, f"tmp/template_{epoch}.pt")
