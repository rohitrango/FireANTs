...
class SyNRegistration(AbstractRegistration):
    '''
    This class implements syn registration with a custom loss
    Both the fixed and moving images are interpolated to a common image
    '''    
    def __init__(self, scales: List[int], iterations: List[float], 
        ...
        if deformation_type == 'geodesic':
            warp = GeodesicShooting(fixed_images, moving_images, integrator_n=integrator_n, optimizer=optimizer, optimizer_lr=optimizer_lr)
        else:
