...
def main(args):
    ...
    otherwise we will chunk the image list, and create averages across all processes, and aggregate them
    ...
    '''
    Outer loop for template creation

    For each epoch, we initialize the batches, keep an estimate of the updated template
    and `init_template_batch` using the `init_template` image object
    '''
    ...
    for epoch in range(args.template_iterations):
        ...
        We take a batch of images, register them to the template, and update the template
        '''
