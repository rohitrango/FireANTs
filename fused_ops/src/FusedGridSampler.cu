    ...
    CUDA_KERNEL_LOOP_TYPE(index, count, index_t) {
        ...
        if (!grid) {
            ...
            if (affine_3d_pregrid) {
                ...
                i_dx = affine_3d_pregrid_ptr[0] * dx + affine_3d_pregrid_ptr[1] * dy + affine_3d_pregrid_ptr[2] * dz ;
                i_dy = affine_3d_pregrid_ptr[3] * dx + affine_3d_pregrid_ptr[4] * dy + affine_3d_pregrid_ptr[5] * dz ;
...
__global__ void fused_grid_sampler_3d_backward_kernel(
    ...
    CUDA_KERNEL_LOOP_TYPE(index, count, index_t) {
        ...
        if (!grid) {
            ...
            grid_t dz = grid[grid_offset + 2];
            grid_t i_dx, i_dy, i_dz;
            // pre-multiply grid displacement
            if (affine_3d_pregrid) {
                const grid_t* affine_3d_pregrid_ptr = affine_3d_pregrid + (broadcast_affine_3d_pregrid ? 0 : (9 * n));
                i_dx = affine_3d_pregrid_ptr[0] * dx + affine_3d_pregrid_ptr[1] * dy + affine_3d_pregrid_ptr[2] * dz;
                i_dy = affine_3d_pregrid_ptr[3] * dx + affine_3d_pregrid_ptr[4] * dy + affine_3d_pregrid_ptr[5] * dz;
        ...
        if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
            ...
            if (affine_3d_pregrid) {
                ...
                paz = gix * affine_3d_pregrid_ptr[2] + giy * affine_3d_pregrid_ptr[5] + giz * affine_3d_pregrid_ptr[8];
                // modified gix, giy, giz
                gix = pax;
                giy = pay;
                giz = paz;
            }
...
torch::Tensor fused_grid_sampler_3d_forward_impl(
    ...
    if (batch_size_max > 1) {
        ...
        // broadcast grid_affine_pre if it exists
        if (affine_3d_pregrid.has_value() && affine_3d_pregrid.value().size(0) == 1) {
    ...
    // check for grid_affine_pre and affine_3d
    if (affine_3d_pregrid.has_value()) {
        TORCH_CHECK(affine_3d_pregrid.value().dim() == 3, "affine_3d_pregrid must be (B, 3, 3)");
        TORCH_CHECK(affine_3d_pregrid.value().device().is_cuda(), "affine_3d_pregrid must be on CUDA");
        TORCH_CHECK(affine_3d_pregrid.value().is_contiguous(), "affine_3d_pregrid must be contiguous");
        TORCH_CHECK(affine_3d_pregrid.value().size(1) == 3, "affine_3d_pregrid must be (B, 3, 3)");
        TORCH_CHECK(affine_3d_pregrid.value().size(2) == 3, "affine_3d_pregrid must be (B, 3, 3)");
    }
    ...
    if (output.has_value()) {
        TORCH_CHECK(output.value().dim() == 5, "output must be 5D");
        ...
        TORCH_CHECK(output.value().size(0) == batch_size_max, "output must be (B, C, D, H, W)");
        TORCH_CHECK(output.value().size(1) == C, "output must be (B, C, D, H, W)");
        TORCH_CHECK(output.value().size(2) == D, "output must be (B, C, D, H, W)");
        TORCH_CHECK(output.value().size(3) == H, "output must be (B, C, D, H, W)");
        TORCH_CHECK(output.value().size(4) == W, "output must be (B, C, D, H, W)");
    } else {
        output.emplace(torch::zeros({batch_size_max, C, D, H, W}, input.options()));
    }
            ...
            grid_scalar_type, "fused_grid_sampler_3d_forward_kernel_sub", [&] {
                ...
                if (canUse32BitIndexMath(input) && grid32bit &&
                    ...
                    <<<GET_BLOCKS(count, 512), 512, 0, stream>>>(
                        ...
                        affine_3d_pregrid.has_value() ? affine_3d_pregrid.value().data_ptr<grid_t>() : nullptr,
...
void fused_grid_sampler_3d_backward_impl(
        ...
        if (batch_size_max > 1) {
            ...
            if (affine_3d_pregrid.has_value() && affine_3d_pregrid.value().size(0) == 1) {
                ...
                TORCH_CHECK(false, "affine_3d_pregrid batch size must match batch size of input or grid");
            }
        }
